  ...
  app.post("/api/generate-exit-scenario", async (req, res) => {
    try {
      ...
      if (!ai) {
        ...
        const isPPWRCase = /ppwr|歐盟|空隙率|dfr|pfas|可回收評級|void\s*space/i.test(kw);
        ...
        if (isMaureenCase) {
          ...
          opening = "「在現職服務期間，我非常感謝主管與團隊給予的高度信任與發揮空間，讓我有機會主導關鍵的自行車全紙化無塑包裝專案，並深入研究歐盟新版包裝與包裝廢棄物法規（PPWR），累積了 SolidWorks 結構建模、ISTA 1A 國際落摔測試認證，以及主動推動跨部門無塑包裝資料庫的實戰經驗。」";
          pivot = "「在深入推進歐盟 PPWR 合規實務的過程中，我體會到新法規對結構工程師帶來的本質顛覆：法規強制運輸包裝空隙率不得超過 50%，且明訂氣泡布、保麗龍等非剛性緩衝材『全數視同空隙』，不能再靠塞滿填充物過落摔；2030 年更強制 DfR 可回收性評級必須達 Grade C（≥80%）以上，並嚴格審查標籤雙面膠 MSDS 水溶性散漿膠系與禁用碳黑 NIR 光學分揀油墨。這類深水區的極限綠色研發，必須依託在出貨量龐大、具備全球供應鏈規模且高度重視專利壁壘的世界級品牌商平台，才能真正產生規模化的經濟與環境效益。」";
          closing = "「Stanley Black & Decker 身為全球五金手工具龍頭，歐洲市場更是集團關鍵戰略版圖，亞洲 GSMA 營運中心更有著極強的研發樞紐動能。我期盼能將我在 3D CAD 最小化空隙率計算、純紙幾何自鎖吸能、單一材質（Mono-material）以及 DoC 5 年技術檔案建立的實戰即戰力，直接投入在史丹利的全球工具產品線中，協助團隊搶先建立符合 2030 PPWR 的最高合規護城河！」";
        ...
        const redlineAvoided = isMaureenCase
          ...
          ? [
              {
                riskyWord: "批評前東家不願投入歐盟合規或不肯花錢開模",
              ...
              {
                riskyWord: "只談泛泛的環保口號卻提不出數據標準",
                safeReplacement: "精準引用「空隙率 ≤50%、DfR Grade C ≥80% 准入門檻、MSDS 水溶性膠水與 5 年 DoC 技術檔案」"
              }
            ]
          : [
              {
