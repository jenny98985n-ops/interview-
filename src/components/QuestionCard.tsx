  ...
  return (
    ...
    >
      ...
      <div className={`p-5 sm:p-6 border-b border-stone-100 ${
        ...
        {question.scenario && (
          <p className="text-xs sm:text-sm text-stone-500 mt-2 bg-stone-100/80 p-2.5 rounded-md border border-stone-200/60 leading-relaxed">
            <strong className="text-stone-700 font-semibold">面試場景意圖：</strong> {question.scenario}
          </p>
        )}
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          {question.tags.map((tag) => (
            <span
              key={tag}
              className="text-[11px] font-medium bg-amber-50 text-amber-900 px-2 py-0.5 rounded border border-amber-200/50"
            >
              #{tag}
            </span>
          ))}
      ...
      {isCardExpanded && (
        <div className="p-5 sm:p-6">
          {/* Sub-tabs */}
          <div className="flex flex-wrap gap-1 border-b border-stone-200 pb-2 mb-4">
            <button
              id={`tab-answer-${question.id}`}
              onClick={() => setActiveSubTab("answer")}
              className={`px-3 py-1.5 text-xs sm:text-sm font-semibold rounded-lg flex items-center space-x-1.5 transition-colors ${
                activeSubTab === "answer"
            ...
            {question.category === "exit_reason" && (
              <button
                ...
                className={`px-3 py-1.5 text-xs sm:text-sm font-semibold rounded-lg flex items-center space-x-1.5 transition-all ${
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="inline-flex items-center space-x-1 text-xs font-medium text-stone-600 hover:text-stone-900 bg-stone-100 hover:bg-stone-200 px-2.5 py-1 rounded transition-colors"
                >
                  {copied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                  <span>{copied ? "已複製至剪貼簿" : "複製擬答"}</span>
                </button>
              </div>

              <div className="bg-stone-50 text-stone-900 p-5 rounded-xl text-sm leading-relaxed whitespace-pre-line font-sans border border-stone-200">
              ...
              {question.spokenDraft && (
                <div className="bg-white border border-stone-200 rounded-xl p-4 space-y-2 shadow-xs">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 text-stone-900 font-bold text-xs">
                      <span className="px-2 py-0.5 rounded bg-stone-100 text-stone-700 text-[11px] font-mono">口語化實戰範例</span>
                      <span className="text-stone-500 text-[11px]">面試現場自然發音表達</span>
                    </div>
                  </div>
                  <div className="text-xs text-stone-800 leading-relaxed whitespace-pre-line bg-stone-50/80 p-3.5 rounded-lg border border-stone-200">
                    {question.spokenDraft}
                  </div>
                </div>
              )}
                ...
                <div className="border border-stone-200 rounded-xl overflow-hidden bg-white shadow-xs">
                  <div className="bg-stone-100/90 px-4 py-2.5 border-b border-stone-200 flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <span className="text-emerald-700 font-bold text-xs">✅ ISTA 測試失敗實例分析庫</span>
                      <span className="text-[11px] text-stone-500 font-medium">（現象 ➔ 原因 ➔ 思考方向 ➔ 解法）</span>
                    </div>
                  </div>
                      ...
                      <tbody className="divide-y divide-stone-200">
                        {question.failureCases.map((fc, idx) => (
                          <tr key={idx} className="hover:bg-stone-50/60 transition-colors align-top">
                            ...
                            <td className="p-3 text-rose-700 font-medium whitespace-pre-line">
                              {fc.phenomenon}
                            </td>
                            <td className="p-3 text-stone-700 whitespace-pre-line">
                              {fc.cause}
                            </td>
                            <td className="p-3 text-stone-600 whitespace-pre-line">
                              {fc.thinking}
                            </td>
                            <td className="p-3 text-emerald-800 font-medium whitespace-pre-line">
                              {fc.solution}
                            </td>
                          </tr>
                        ))}
              ...
              {question.tableSections && question.tableSections.map((sec, sIdx) => (
                <div key={sIdx} className="border border-stone-200 rounded-xl overflow-hidden bg-white shadow-xs">
                  <div className="bg-stone-100/80 px-4 py-2 border-b border-stone-200 font-bold text-xs text-stone-900">
                    {sec.title}
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-stone-50 text-stone-700 font-semibold border-b border-stone-200">
                        <tr>
                          {sec.headers.map((h, hIdx) => (
                            <th key={hIdx} className="p-2.5 whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-stone-200">
                        {sec.rows.map((row, rIdx) => (
                          <tr key={rIdx} className="hover:bg-stone-50/50 align-top">
                            {row.map((cell, cIdx) => (
                              <td key={cIdx} className="p-2.5 text-stone-800 whitespace-pre-line">
                                {cell}
                              </td>
                            ))}
                          </tr>
              ...
              {question.stanleyContext && (
                <div className="bg-amber-50/70 border-l-4 border-amber-400 p-3 rounded-r-lg text-xs text-amber-950 leading-relaxed">
              ...
              {question.englishPhrasing && (
                <div className="bg-stone-950 text-stone-200 rounded-xl p-3.5 border border-stone-800 space-y-1">
          ...
          {activeSubTab === "star" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
              <div className="bg-stone-50 border border-stone-200 rounded-lg p-3.5">
                <div className="flex items-center space-x-2 text-stone-900 font-bold text-xs mb-1.5">
                ...
                <p className="text-xs text-stone-700 leading-relaxed">{question.star.situation}</p>
              </div>

              <div className="bg-stone-50 border border-stone-200 rounded-lg p-3.5">
                <div className="flex items-center space-x-2 text-stone-900 font-bold text-xs mb-1.5">
                  <span className="w-5 h-5 rounded-full bg-amber-100 text-amber-900 flex items-center justify-center text-[11px]">
                    T
                  </span>
                  <span>Task (核心任務與限制)</span>
                </div>
                <p className="text-xs text-stone-700 leading-relaxed">{question.star.task}</p>
              </div>

              <div className="bg-stone-50 border border-stone-200 rounded-lg p-3.5">
                <div className="flex items-center space-x-2 text-stone-900 font-bold text-xs mb-1.5">
                  <span className="w-5 h-5 rounded-full bg-purple-100 text-purple-900 flex items-center justify-center text-[11px]">
                    A
                  </span>
                  <span>Action (關鍵行動與技術突破)</span>
                </div>
                <p className="text-xs text-stone-700 leading-relaxed">{question.star.action}</p>
              </div>

              <div className="bg-stone-50 border border-stone-200 rounded-lg p-3.5">
                <div className="flex items-center space-x-2 text-stone-900 font-bold text-xs mb-1.5">
                  <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-900 flex items-center justify-center text-[11px]">
                    R
                  </span>
                  <span>Result (量化成果與商業價值)</span>
                </div>
                <p className="text-xs text-stone-700 leading-relaxed">{question.star.result}</p>
              </div>
            </div>
          )}
          ...
          {activeSubTab === "strategy" && (
            <div className="space-y-4">
              <div className="bg-amber-50/60 border border-amber-200/80 rounded-xl p-4 text-xs sm:text-sm text-stone-800 leading-relaxed">
              ...
              <div>
                <h4 className="text-xs font-bold text-stone-700 mb-2 uppercase tracking-wide">
                  建議提及之包裝工程專業關鍵詞：
                </h4>
                <div className="flex flex-wrap gap-2">
                  {question.keyTerms.map((term) => (
                    <span
                      key={term}
                      className="px-2.5 py-1 rounded-md bg-stone-100 text-stone-800 font-mono text-xs border border-stone-200"
                    >
                      {term}
                    </span>
                  ))}
          ...
          {activeSubTab === "pitfalls" && (
            <div className="space-y-3">
              <div className="text-xs font-bold text-red-900 flex items-center space-x-1.5 mb-1">
              ...
              <ul className="space-y-2">
                {question.pitfalls.map((pitfall, idx) => (
                  <li
                    key={idx}
                    className="flex items-start space-x-2 text-xs text-stone-800 bg-red-50/70 p-3 rounded-lg border border-red-200/60 leading-relaxed"
                  >
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="text-xs font-semibold bg-stone-900 text-white px-3 py-1 rounded hover:bg-stone-800 transition-colors"
                >
                  儲存筆記
                </button>
              </div>
              <textarea
                ...
                rows={5}
                className="w-full text-xs sm:text-sm p-3 border border-stone-300 rounded-lg focus:outline-hidden focus:ring-2 focus:ring-amber-400 focus:border-amber-400 font-sans"
              />
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="text-xs font-medium text-amber-900 bg-amber-100 hover:bg-amber-200 border border-amber-300 px-3 py-1 rounded flex items-center space-x-1"
                >
                  {isAiLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 text-amber-600" />}
              ...
              {!aiResult && !isAiLoading && (
                <div className="text-center py-6 bg-stone-50 rounded-xl border border-dashed border-stone-300 p-4">
                  ...
                  <button
                    ...
                    onClick={handleRequestAiCritique}
                    className="inline-flex items-center space-x-2 bg-stone-900 hover:bg-stone-800 text-amber-400 font-semibold text-xs px-4 py-2 rounded-lg shadow-xs"
                  >
              ...
              {aiResult && (
                <div className="bg-stone-900 text-stone-100 rounded-xl p-5 border border-stone-800 space-y-4">
                  <div className="flex items-center justify-between border-b border-stone-800 pb-3">
                    <div className="flex items-center space-x-2">
                      <span className="text-xs text-stone-400">總體評分</span>
                      <span className="text-xl font-bold text-amber-400">{aiResult.score} / 100</span>
                    </div>
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-stone-300 mb-1">總監點評總結：</h5>
                    <p className="text-xs sm:text-sm text-stone-200 leading-relaxed">{aiResult.summary}</p>
                  </div>
                  ...
                  {aiResult.modelAnswerHighlight && (
                    <div className="bg-amber-400/10 border border-amber-400/30 p-3 rounded-lg text-xs text-amber-200">
                  ...
                  {aiResult.followUpQuestion && (
                    <div className="bg-stone-800 p-3.5 rounded-lg border border-amber-400/40">
                      <div className="text-xs font-bold text-amber-400 mb-1 flex items-center space-x-1">
                        <Target className="w-3.5 h-3.5" />
                        <span>現場可能遭遇的深度追問 (Follow-up Challenge)：</span>
                      </div>
                      <p className="text-xs sm:text-sm text-white font-medium">{aiResult.followUpQuestion}</p>
                    </div>
                  )}
